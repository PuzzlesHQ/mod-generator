...
onMounted(async () => {
  ...
  crVersions.value = versions.crVersions;
  pCosVersions.value = versions.pCosVersions;
  pCorVersions.value = versions.pCorVersions;
  pJigVersions.value = versions.pJigVersions;
  if (crVersions.value.length > 0) state.crVersion = crVersions.value[0];
  if (pCosVersions.value.length > 0) state.pCosVersion = pCosVersions.value[0];
  if (pCorVersions.value.length > 0) state.pCorVersion = pCorVersions.value[0];
  if (pJigVersions.value.length > 0) state.pJigVersion = pJigVersions.value[0];
});
...
const zipName = computed(
  () => `${computeModId(state.modId)}-template-${state.crVersion}.zip`,
);
...
const validations = {
  modName: {
    ...
    minLength: helpers.withMessage(
      "Mod name should have at least 3 characters",
      minLength(3),
    ),
  },
  modId: {
    required,
    minLength: helpers.withMessage(
      "Mod ID must have at least 2 characters",
      minLength(2),
    ),
    maxLength: helpers.withMessage(
      "Mod ID must have at most 64 characters",
      maxLength(64),
    ),
    // Regex sourced from FML
    pattern: helpers.withMessage(
      "Invalid mod ID",
      helpers.regex(/^(?=.{2,64}$)[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)*$/),
    ),
  },
  packageName: {
    required,
    ...
    pattern: helpers.withMessage(
      ...
      helpers.regex(/^[a-z][a-z0-9_]*(\.[a-z0-9_]+)+[0-9a-z_]$/),
    ),
  },
};
...
const submit = async (generator: () => Promise<any>) => {
  ...
  if (isCorrect) {
    overlay.value = true;
    await generator().finally(() => (overlay.value = false));
  } else {
    ...
    <v-form @submit.prevent>
      <v-text-field
        ...
        @input="
          v$.modName
            .$validate()
            .then(() => (automaticModId ? v$.modId.$validate() : undefined))
        "
        variant="outlined"
        class="rounded"
      />
      ...
      <v-text-field
        ...
        :bg-color="
          v$.modId.$errors.length > 0 && automaticModId ? 'red' : undefined
        "
      />
      <v-checkbox
      ...
      <v-text-field
        ...
        required
        :error-messages="
          v$.packageName.$errors.map((e) => e.$message as string)
        "
